...
public final class EventLogWriter {
        ...
        try {
            if (!dir.equals(logOldPath)) {
                ...
                if (scenName != null && !scenName.equals(""))
                    handler = new FileHandler(dir + "EventLog_" + scenName.substring(0, scenName.length() - 4) + "_" + df.format(dt) + "." + format, true);
                else
                    handler = new FileHandler(dir + "EventLog_" + df.format(dt) + "." + format, true);
                logOldPath = dir;
    ...
    public static void setLogPath(String logPath) {
        ...
        EventLogWriter.logPath = logPath;
    }
